...
for key, dist in enumerate(H2_dist):
    ...
    molecule = driver.run()
    active_spatial_orbitals = molecule.num_orbitals
    num_orbitals = 2*active_spatial_orbitals
    num_qubits = num_orbitals
    print('{} is the distance'.format(dist))
    print(geometry.format(dist))
    # driver = PySCFDriver(atom=geometry.format(dist), unit=UnitsType.ANGSTROM, charge=0, spin=0, basis=basis)
    print('{} orbitals'.format(molecule.num_orbitals))
    orb_deletion = [i for i in range(active_spatial_orbitals, molecule.num_orbitals)]
    num_particles = molecule.num_beta + molecule.num_alpha
    print('{} particles.'.format(num_particles))
    print('Instantiating Hamiltonian')
    one_electron_spatial_integrals = molecule.mo_onee_ints[:active_spatial_orbitals, :active_spatial_orbitals]

    two_electron_spatial_integrals = molecule.mo_eri_ints[:active_spatial_orbitals, :active_spatial_orbitals, :active_spatial_orbitals,
    ...
    print('{} atoms'.format(molecule.num_atoms))
    ...
    qm.mo_eri_ints = molecule.mo_eri_ints[:active_spatial_orbitals,:active_spatial_orbitals,:active_spatial_orbitals,:active_spatial_orbitals]
    ...
    # print('The exact energy is {}'.format(exact_energy))


    ##################### RUN VQE ######################
    active_occ = num_particles // 2
    active_virt = (num_orbitals - num_particles) // 2
    ...
    init_state = HartreeFock(num_qubits, num_orbitals, num_particles, map_type,two_qubit_reduction=False)

    var_op = UCCSD(num_qubits=num_qubits, depth=1, num_orbitals=num_orbitals, num_particles=num_particles, active_occupied=active_occ_list,\
                   active_unoccupied=active_virt_list,initial_state=init_state, qubit_mapping=map_type,
                   two_qubit_reduction=False, mp2_reduction=False, singles_deletion=False)
    ...
    print('Circuit depth: ',var_cirq.depth())

    # setup a classical optimizer for VQE
    # max_eval = 1000
    # optimizer = COBYLA(maxiter=max_eval,disp=True, tol=1e-4,  rhobeg=1e-1)
    ...
    # vqe_energy = result['energy']+molecule.nuclear_repulsion_energy
    # print('The VQE energy is: ',result['energy']+molecule.nuclear_repulsion_energy)
    # end_time = time.time()
