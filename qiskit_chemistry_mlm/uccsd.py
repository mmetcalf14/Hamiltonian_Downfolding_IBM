...
class UCCSD(VariationalForm):
    ...
    def __init__(self, num_qubits, depth, num_orbitals, num_particles,
                 ...
                 qubit_mapping='parity', two_qubit_reduction=True, mp2_reduction=False, singles_deletion=False,
                 num_time_slices=1, cliffords=None, sq_list=None, tapering_values=None, symmetries=None,
        ...
        print('Here are the singles: ', self._single_excitations)
        print('There are {} are doubles'.format(len(self._double_excitations)))

        if mp2_reduction:
            self._double_excitations, self._mp2_coeff = get_mp2_doubles(self._single_excitations,self._double_excitations)
        else: self._mp2_coeff = None

        print('There are {} are the new doubles'.format(len(self._double_excitations)))
        print(self._double_excitations)
    ...
    def construct_circuit(self, parameters, q=None, compile = True):
        """
        ...
        for qc in results:
            if self._shallow_circuit_concat:
                ...
                circuit += qc
        # Add a pytket compiling routine in here. Maybe develop a little function that
        # returns the compiled circuit
        if compile == True:
            circuit = pytket_compiled_circuit(circuit)
        return circuit
    ...
    def _construct_circuit_for_one_excited_operator(qubit_op_and_param, qr, num_time_slices):
        ...
        # This might be a good place for the compiler? Or does it work better with larger circuits
        # Worth looking into
        return qc
...
def get_mp2_doubles(single_excitations, double_excitations):
    ...
    mp2 = MP2Info(qm, single_excitations, double_excitations, threshold=1e-12)
    mp2_doubles = mp2._mp2_doubles
    mp2_coeff = mp2._mp2_coeff


    return mp2_doubles, mp2_coeff

# def pytket_compiled_circuit(circuit):
#     # Optimize - CCQ
#     from pytket.qiskit import qiskit_to_tk, tk_to_qiskit
#     from pytket import Transform
#
#     tk_cirq = qiskit_to_tk(circuit)
