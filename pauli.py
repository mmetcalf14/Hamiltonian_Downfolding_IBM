...
class Pauli:
    ...
    def to_spmatrix_reduced(self, indices, num_qubits):
        _x, _z = self._x, self._z
        n = 2 ** len(_x)
        top_val = indices[-1]
        temp_indices = indices.copy()
        index_length = len(indices)
        if len(indices) != 2 ** num_qubits:
            residue_num = 2 ** num_qubits - len(indices)
            residue = [top_val + k for k in range(1, residue_num + 1)]
            temp_indices.extend(residue)

        new_n = 2**num_qubits
        twos_array = 1 << np.arange(len(_x))
        xs = np.array(_x).dot(twos_array)
        zs = np.array(_z).dot(twos_array)
        old_rows = np.arange(n + 1, dtype=np.uint)
        old_columns = old_rows ^ xs
        global_factor = (-1j) ** np.dot(np.array(_x, dtype=np.uint), _z)
        old_data = global_factor * (-1) ** np.mod(_count_set_bits(zs & old_rows), 2)
        data_lst = []
        columns_lst = []
        rows = np.arange(new_n + 1, dtype=np.uint)
        indices_dict = {}
        # print('{} is the index length'.format(len(indices)))
        for i in range(len(temp_indices)):
            indices_dict.update({temp_indices[i]: i})
        # print(indices_dict)
        # We need to add some sort of identity when the dimension is not 2**N
        # Can this be done in this mapping scheme. Extra bits can be determined using base 2
        for i in temp_indices:
            #print(i)
            if i <= top_val:
                data_lst.append(old_data[i])
            else: data_lst.append(0)

            if indices_dict.get(old_columns[i]) is not None:
                #print('Old cols check')
                columns_lst.append(indices_dict.get(old_columns[i]))
            else:
                # columns_lst.append(old_columns[i])
                columns_lst.append(0)
        for i in range(len(temp_indices)):
            indices_dict.update({temp_indices[i]: i})

        columns_lst.append(len(columns_lst))
        data_lst.append(len(data_lst))
        columns = np.array(columns_lst)
        data = np.array(data_lst)
        # print('spmatrix')
        return sparse.csr_matrix((data, columns, rows), shape=(new_n, new_n))
    # def to_spmatrix_reduced(self, indices, num_qubits):
    #     _x, _z = self._x, self._z
    #     n = 2 ** len(_x)
    #     new_n = num_qubits ** 2
    ...
    #         indices_dict.update({indices[i]: i})
    #     col_dict = {}
    #     for i in range(len(old_columns)):
    #         col_dict.update({old_columns[i]: i})
    #     for i in indices:
    #         data_lst.append(old_data[i])
    #         if indices_dict.get(old_columns[i]) is not None:
    #             columns_lst.append(indices_dict.get(old_columns[i]))
