...
class MP2Info:
    ...
    def __init__(self, qmolecule, single_excitations, double_excitations, threshold=1e-12):
        """
        ...
        """
        self._single_excitations = single_excitations
        self._double_excitations = double_excitations
        self._terms, self._mp2_delta, self._mp2_doubles, self._mp2_coeff = self._compute_mp2(qmolecule, threshold)
    ...
    def _compute_mp2(self, qmolecule, threshold):
        terms = {}
        mp2_delta = 0
        mp2_coeff = []
        ...
        # There is an issue using this with UCCSD, I think you need to modify
        # singles = self._single_excitations
        doubles = self._double_excitations
        # print('{} are the double excitations: ', self._double_excitations)
        ...
        for n in range(len(doubles)):
            ...
            if coeff != 0.0:
                mp2_coeff.append(coeff)
                new_doubles.append(idxs)
            e_delta = coeff * tiajb
            e_delta = e_delta if abs(e_delta) > threshold else 0
            # print('Doubles: ', idxs, ' and coeff: ', coeff)
            terms[_list_to_str(idxs)] = (coeff, e_delta)
            mp2_delta += e_delta

        return terms, mp2_delta, new_doubles, mp2_coeff
